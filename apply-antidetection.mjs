// apply-antidetection.mjs — Anti-Bot Stealth + Dashboard UI Visual Badges
...
const patches = [
  {
    name: "1/13 import fingerprint module",
  ...
  {
    name: "2/13 anti-detection config + helpers",
    find: L(
      `if (!SESSION_SECRET) {`,
  ...
  {
    name: "3/13 per-account browser profile (createAccount base)",
  ...
  {
    name: "4/13 keep profile on restore",
  ...
  {
    name: "5/13 route all Kick fetches through impersonatedFetch",
    find: L(
  ...
  {
    name: "6/13 human delay in sendKick",
    find: L(
  ...
  {
    name: "7/13 human delay in sendKickIsolated",
  ...
  {
    name: "8/13 expose fingerprint in publicAccount",
  ...
  {
    name: "9/13 status endpoint + startup log",
  ...
  {
    name: "10/13 log fingerprint on account creation",
  ...
  {
    name: "11/13 anti-detection status in startup log",
  ...
  {
    name: "12/13 anti-bot shield badge on account cards + header",
    find: `      <div class=\\"accountIsolationId\\">ID \\\${escapeHtml(String(srv.id||\\"\\").slice(0,8)||\"pending\")} • \\\${escapeHtml(srv.isolation?.sessionNamespace||\"session pending\")}</div>`,
    replace: `      <div class=\\"accountIsolationId\\">ID \\\${escapeHtml(String(srv.id||\\"\\").slice(0,8)||\"pending\")} • \\\${escapeHtml(srv.isolation?.sessionNamespace||\"session pending\")}</div>\\n      <div style=\\"margin:6px 0;padding:6px 10px;background:rgba(0,240,255,0.06);border:1px solid rgba(0,240,255,0.25);border-radius:8px;font-size:10px;font-family:ui-monospace,SFMono-Regular,monospace;color:#a0ecff;display:flex;justify-content:space-between;align-items:center;\\"><span>🛡️ \\\${escapeHtml(srv.fingerprint?.short||\"Spoofed Profile Pending\")}</span><span style=\\"color:\\\${srv.fingerprint?.tls?.available?\"#00ff66\":\"#e6c55f\"};font-weight:bold;\\">\\\${escapeHtml(srv.fingerprint?.tls?.available?\"TLS Impersonated\":\"Header Fallback\")}</span></div>`,
    guard: `🛡️`
  },
  {
    name: "13/13 Cyberpunk HUD Visual Upgrade",
    find: `</style>`,
    replace: L(
...
console.log(`\nDone — ${applied}/13 patches applied.`);
